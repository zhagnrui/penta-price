...
type AcidOption = {
  ...
  labelEn: string
  mw: number          // g/mol (or effective MW for blends)
}
...
const ACID_OPTIONS: Record<string, AcidOption> = {
  ...
  c8c10: { label: '混合 C8/C10 (1:1 blend)', labelEn: 'Mixed C8/C10 blend (1:1 mol)', mw: (144.21 + 172.26) / 2 },
}

// ── Molecular weights ───────────────────────────────
const MW_PE    = 136.15   // g/mol
const MW_WATER = 18.015   // g/mol

// ── Default values ───────────────────────────────────
const DEFAULTS = {
  peKg: 100,
  acidKey: 'c8' as string,
  molarRatio: 4.2,
  purity: 98,
}
...
// ── Main Component ───────────────────────────────────
export default function LubricantCalc() {
  ...
  const result = useMemo(
    () => calculate(peKg, acid.mw, molarRatio, purity),
    [peKg, acid.mw, molarRatio, purity],
  )

  const handleReset = () => {
    setPeKg(DEFAULTS.peKg)
    setAcidKey(DEFAULTS.acidKey)
    setMolarRatio(DEFAULTS.molarRatio)
    ...
    <div className="pe-calc-wrapper">
      ...
      <div className="pe-calc-panel pe-card">
        ...
        <div className="pe-calc-fields">
          <NumInput
            label="PE 用量"
            labelEn="Pentaerythritol amount"
            value={peKg}
            onChange={setPeKg}
            min={1}
            max={100000}
          ...
          <label className="pe-calc-field">
            <span className="pe-calc-label">
              脂肪酸种类
              <span className="pe-calc-label-en">Fatty acid type</span>
            </span>
            <div className="pe-calc-input-wrap">
              <select
                className="pe-calc-input pe-calc-select"
                value={acidKey}
                onChange={e => setAcidKey(e.target.value)}
              >
                {Object.entries(ACID_OPTIONS).map(([key, opt]) => (
          ...
          <NumInput
            label="摩尔比 (脂肪酸 : PE)"
            labelEn="Molar ratio (acid : PE)"
            value={molarRatio}
            onChange={setMolarRatio}
            min={4.0}
            max={5.0}
            step={0.1}
            unit=": 1"
      ...
      <div className="pe-calc-panel pe-card">
        ...
        <div className="pe-calc-results">
          <ResultRow
            label="脂肪酸需求量"
            labelEn="Fatty acid required"
            value={fmt(result.acidKg)}
            unit="kg"
          />
          <ResultRow
            label="理论四酯产量"
            labelEn="Theoretical tetraester yield"
        ...
        <div className="pe-calc-disclaimer">
          ...
          <span style={{ opacity: 0.75 }}>
            Results are for reference only; actual yield depends on reaction conditions.
            数据来源：PentaPrice
          </span>
        </div>
      </div>
